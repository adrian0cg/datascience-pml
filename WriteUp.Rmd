...
Likewise, for certain prediction algorithms, which are not model bound, it would be a bad unwise to leave predictors in the dataset, that have unique values for most entries like the `X`, `new_window`, `num_window`, `raw_timestamp_part_1` that could lead to overfitting. Also `cvtd_timestamp` is somewhat synonymous to `user_name`. Thus we retain as simplified dataset:

```{r removeOverfitters, echo=TRUE, cache=TRUE}
removeOverfitters <- rep(TRUE,160); overfitters[c(1,3,5,7)] = FALSE;
pmlTrain <- pmlTraining[, !(nzvPml$zeroVar | nzvPml$nzv) & measuredColumns & removeOverfitters]
